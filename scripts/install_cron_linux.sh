...
#   08:15 on the 1st-3rd  monthly_billing_snapshot.sh  closed month's cumulative snapshot
...
#                     Collection vs Billing reads "billing not loaded yet". Repeated on
#                     the 2nd and 3rd because the ERP sometimes takes a day or two to
#                     finish generating bills; each run reloads the same two labels, so
#                     a later run simply overwrites an earlier partial one.
#
...
monthly_entry() {  # monthly_entry "HH:MM-IST" "jobname" "command…" [days-of-month]
  ...
  local t="$1" name="$2" cmd="$3" days="${4:-1}"
  local H M
  H="$(ist "$t" %H)"; M="$(ist "$t" %M)"
  echo "$M $H $days * * cd '$REPO_DIR' && flock -n /tmp/patrika_${name}.lock $cmd >> '$LOG_DIR/cron_${name}.log' 2>&1"
}
...
$(monthly_entry "08:15" "monthly_billing" "./scripts/monthly_billing_snapshot.sh" "1-3")
$MARK_END"
