...
module.exports = function installAgentApp({ app, q }) {
  ...
  // GET /api/agent/supply?unit=&agcd=&days=30 — recent supply trend + latest breakdown
  app.get('/api/agent/supply', async (req, res) => {
    try {
      ...
      const [trend, latest] = await Promise.all([
        q(`SELECT supply_date, SUM(sup_copy) copies, SUM(sup_copy*sup_rate) value
             FROM supply_data
            WHERE unit_code=? AND agcd=? AND supply_date >= DATE_SUB(CURDATE(), INTERVAL ? DAY)
            GROUP BY supply_date ORDER BY supply_date`, [unit_code, agcd, days]),
        q(`SELECT MAX(supply_date) mx FROM supply_data WHERE unit_code=? AND agcd=?`, [unit_code, agcd]),
      ]);
      let breakdown = [];
      const mx = latest.rows[0] && latest.rows[0].mx ? iso(latest.rows[0].mx) : null;
      if (mx) {
        const b = await q(
          `SELECT publ_name, edtn_name, sup_type_code, supply_type_name,
                  SUM(sup_copy) copies, AVG(sup_rate) rate, AVG(comm_rate) comm
             FROM supply_data WHERE unit_code=? AND agcd=? AND supply_date=?
             GROUP BY publ_name, edtn_name, sup_type_code, supply_type_name
             ORDER BY copies DESC`, [unit_code, agcd, mx]);
        breakdown = b.rows.map(r => ({
          publication: r.publ_name, edition: r.edtn_name, type: r.supply_type_name,
          copies: num(r.copies), rate: num(r.rate), commission: num(r.comm),
        }));
      }
      res.json({
        agency, latest_supply_date: mx,
