...
module.exports = function installCommandCentre({ app, q, getScopeUnitCodes }) {
  ...
  /* The Agent / Cash toggle reaches these charts too.
     ...
     ALL THREE SEGMENTS COME FROM ONE PASS. Scanning per segment meant the two-year
     supply scan ran again every time the toggle moved — about 25 seconds each, three
     times over, and the screen sat on skeletons for the whole of it. Agent and cash are
     read once and the three views are derived in memory, so switching the toggle costs
     ...
     Collection splits by agency class in the same single pass. City sale is settled at
     the centre rather than through an agency ledger, so the cash series is genuinely
     near-empty — a fact about the business, which the chart states rather than leaving
     the reader to guess. */
  async function quarterly(asOn, unitScope) {
    ...
    const CRED = `(SELECT DISTINCT unit, agcd FROM agency_master WHERE ag_class_name = 'CREDIT SALE')`;

    const [coll, supA, supH] = await Promise.all([
      /* Receipts banked, labelled by the class of the agency that paid, so agent and
         cash can be told apart without a second scan. */
      q(`SELECT YEAR(c.coll_date) y, QUARTER(c.coll_date) q,
                CASE WHEN cm.agcd IS NULL THEN 'cash' ELSE 'agent' END seg,
                -COALESCE(SUM(c.amount),0) amt
           FROM agency_collection c
           LEFT JOIN ${CRED} cm ON cm.unit = c.unit_code AND cm.agcd = c.ag_code
          WHERE c.is_valid=1 AND c.coll_date BETWEEN ? AND ?${_ufC.cl}
          GROUP BY y, q, seg`, [fyBase.from, fyCur.to, ..._ufC.p]),
      ...
      q(`SELECT YEAR(s.supply_date) y, QUARTER(s.supply_date) q, s.unit_code,
           ...
           FROM supply_data s
           JOIN ${CRED} cm ON cm.unit = s.unit_code AND cm.agcd = s.agcd
    ...
    const collBy = { all: mk(), agent: mk(), cash: mk() };
    coll.rows.forEach(r => {
      const f = toFy(N(r.y), N(r.q));
      const put = which => {
        const slot = collBy[which][f.q - 1]; if (!slot) return;
        if (f.fy === fy) slot.current += N(r.amt); else if (f.fy === fy - 1) slot.base += N(r.amt);
      };
      put('all'); put(r.seg === 'cash' ? 'cash' : 'agent');
    });

    const supBy = { all: mk(), agent: mk(), cash: mk() };
    const addSup = (rows, which) => rows.forEach(r => {
      const avg = N(r.days) ? Math.round(N(r.copies) / N(r.days)) : 0;
      [which, 'all'].forEach(k => {
        const slot = supBy[k][N(r.q) - 1]; if (!slot) return;
        if (N(r.y) === cy) slot.current += avg; else if (N(r.y) === cy - 1) slot.base += avg;
      });
    });
    addSup(supA.rows, 'agent'); addSup(supH.rows, 'cash');

    const label = { all: 'Agent + cash', agent: 'Agent sale (credit)', cash: 'Cash sale (city)' };
    const view = seg => ({
      seg,
      collection: collBy[seg], supply: supBy[seg],
      collection_current: `FY ${fy}-${String(fy + 1).slice(2)}`,
      collection_base: `FY ${fy - 1}-${String(fy).slice(2)}`,
      collection_basis: `${label[seg]} · Financial year · Apr–Mar`,
      collection_note: seg === 'cash'
        ? 'City sale is settled at the centre, so little of it reaches the agency ledger'
        : null,
      supply_current: `CY ${cy}`,
      supply_base: `CY ${cy - 1}`,
      supply_basis: `${label[seg]} · Calendar year · Jan–Dec`,
    ...
    return { all: view('all'), agent: view('agent'), cash: view('cash') };
  }

  /* The quarterly figures are a two-year scan — about 25 seconds cold, which was the
     whole reason the Command Centre sat on skeletons. They move only when supply syncs,
     so the result is cached per (as-on date, unit scope) — no longer per segment, since
     one pass now produces all three — and warmed in the background at boot so the first
     person through the door is not the one who pays for it. */
  ...
  async function quarterlyCached(asOn, unitScope, seg) {
    ...
    if (hit && Date.now() - hit.at < QTR_TTL_MS) return hit.val[seg] || hit.val.all;
    const val = await quarterly(asOn, unitScope);
    QTR_CACHE.set(key, { at: Date.now(), val });
    if (seg && val[seg]) return val[seg];
  ...
  /* Warmed in the background so the first request of the day is served from memory.
     Kicked off shortly after boot and refreshed just inside the cache's own lifetime;
     a failure is swallowed on purpose, because a warm-up that cannot run must never
     take the API down with it. */
  async function warmCaches() {
    try {
      const { asOn } = await resolveDates(null, 'prev_year');
      await Promise.all([unitHomeState(), unitCircIncharge(), stateHeads()]);
      await quarterlyCached(asOn, null, 'all');
    } catch (_) { /* the next request will pay for it instead */ }
  }
  setTimeout(warmCaches, 4000).unref?.();
  setInterval(warmCaches, QTR_TTL_MS - 5 * 60 * 1000).unref?.();
  ...
  async function collectionByState(win, asOn, prev, unitScope) {
    ...
    const [mtd, prevDay, ytd, lyYtd, billing] = await Promise.all([
      ...
      /* Year-to-date and the same period last year are anchored on the as-on date and
         do not move with the selected range, yet they were re-scanned on every range
         switch — 2.4s and 1.4s over 1.1M collection rows each time, for two figures
         that cannot have changed. Memoised on (dates, scope) so switching This Month /
         Last Month / FY no longer pays for them again. */
      memo(`ytd|${yStart}|${asOn}|${JSON.stringify(unitScope || '')}`, () =>
        q(`SELECT unit_code unit, -COALESCE(SUM(amount),0) amt FROM agency_collection
           WHERE is_valid=1 AND coll_date BETWEEN ? AND ?${uCl} GROUP BY unit_code`, [yStart, asOn, ...uP])),
      memo(`lyytd|${lyStart}|${lyAsOn}|${JSON.stringify(unitScope || '')}`, () =>
        q(`SELECT unit_code unit, -COALESCE(SUM(amount),0) amt FROM agency_collection
